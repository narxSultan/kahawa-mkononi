...
class AuthState {
  ...
  const AuthState.authenticated(
      {required User user, Customer? customer, AuthEvent? event})
      : this(
...
class AuthController extends StateNotifier<AuthState> {
  ...
  Future<String?> login(
    ...
    try {
      ...
      final profile = me.roleName == 'CUSTOMER'
          ? await ref.read(authRepositoryProvider).myCustomerProfile()
          : null;
      state = AuthState.authenticated(
          user: me, customer: profile, event: AuthEvent.loginSuccess);
      return null;
  ...
  Future<String?> register({
    ...
    try {
      ...
      final profile =
          await ref.read(authRepositoryProvider).myCustomerProfile();
      state = AuthState.authenticated(
          user: me, customer: profile, event: AuthEvent.registerSuccess);
      return null;
    } on ApiException catch (e) {
      state = AuthState.unauthenticated(e.message);
      return state.error ?? e.message;
  ...
  void clearEvent() {
    ...
    if (state.status == AuthStatus.authenticated && state.user != null) {
      state =
          AuthState.authenticated(user: state.user!, customer: state.customer);
      return;
    }
    state = AuthState(
        status: state.status,
  ...
  Future<void> updateProfilePhoto(String profilePhotoUrl) async {
    ...
    final me = await ref
        .read(authRepositoryProvider)
        .updateMyAccount(profilePhoto: profilePhotoUrl);
    state = AuthState.authenticated(user: me, customer: state.customer);
  }
