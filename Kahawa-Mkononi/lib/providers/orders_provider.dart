...
class OrdersController extends AutoDisposeAsyncNotifier<List<Order>> {
  ...
  Future<void> replyToOrderMessage({required String orderId, required String message}) async {
    await ref.read(orderRepositoryProvider).customerMessage(orderId: orderId, message: message);
    await reload();
  }
