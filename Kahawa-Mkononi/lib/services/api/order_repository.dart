...
''',
  ...
  Future<Order> customerMessage({required String orderId, required String message}) async {
    final data = await api.mutate(
      r'''
mutation CustomerMessage($orderId: ID!, $message: String!) {
  customerMessageOrder(orderId:$orderId, message:$message) {
    id status createdAt updatedAt
    transferredAt
    transferredFromServiceCentre { id centreName locationName phone status }
    transferredToServiceCentre { id centreName locationName phone status }
...
''',
    ...
    return Order.fromJson(data['customerMessageOrder'] as Map<String, dynamic>);
  }
