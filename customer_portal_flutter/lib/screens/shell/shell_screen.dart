...
class _ShellScreenState extends ConsumerState<ShellScreen> {
  ...
  Future<void> _maybeShowAuthSuccess(AuthState next) async {
    ...
    final title = event == AuthEvent.registerSuccess ? 'Account created' : 'Welcome back';
    final message = event == AuthEvent.registerSuccess ? 'Registration successful.' : 'Login successful.';
      ...
      final ok = await showDialog<bool>(
        ...
        builder: (c) => AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(onPressed: () => Navigator.of(c).pop(true), child: const Text('Submit')),
          ],
        ),
      );
      if (ok != true) {
  ...
  Future<void> _showConfirmDialog(List<Order> orders) async {
    ...
    try {
      final centre = candidate.serviceCentre?.centreName ?? candidate.transferredToServiceCentre?.centreName ?? '—';
      final action = await showDialog<bool>(
        ...
        builder: (c) => AlertDialog(
          ...
          content: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Order ${candidate.id.substring(0, 6)}', style: const TextStyle(fontWeight: FontWeight.w900)),
              const SizedBox(height: 6),
              Text('Centre: $centre'),
              const SizedBox(height: 6),
              Text('Ready at: ${Formatters.dateTime(candidate.staffCompletedAt ?? candidate.updatedAt)}'),
              const SizedBox(height: 10),
              const Text('Has your order been completed? You must confirm to continue using the app.', style: TextStyle(color: Colors.black87)),
            ],
          ),
          actions: [
            TextButton(onPressed: () => Navigator.of(c).pop(false), child: const Text('Not completed')),
            ElevatedButton(onPressed: () => Navigator.of(c).pop(true), child: const Text('Yes, completed')),
          ],
        ),
      );
      ...
      if (action == true) {
        ...
        if (!mounted) return;
        messenger.showSnackBar(const SnackBar(content: Text('Thank you for confirming.')));
        ...
        await ref.read(ordersProvider.notifier).reject(orderId: candidate.id, reason: reason);
        if (!mounted) return;
        messenger.showSnackBar(const SnackBar(content: Text('Submitted. We will review your issue.')));
      }
    } finally {
      _confirming = false;
      _confirmingOrderId = null;
  ...
  Widget build(BuildContext context) {
    final unread = ref.watch(unreadCountProvider).maybeWhen(data: (v) => v, orElse: () => 0);
                ...
                if (unread > 0)
                  Positioned(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: const Color(0xFFC62828), borderRadius: BorderRadius.circular(999)),
                      child: Text(
                        unread > 99 ? '99+' : '$unread',
